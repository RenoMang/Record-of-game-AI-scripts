...
class Agent(AgentInterface):
    ...
    def beyes_select(self, state: OthelloState):
        ...
        for state in self.children[state]:
            if tuple(state.grid) not in self.prior.keys():
                self.weighted_score(state)

            alpha = self.prior[tuple(state.grid)][0] + self.Q[state]
            beta = self.prior[tuple(state.grid)][1] + self.N[state] - self.Q[state]
            beyes[state] = p * (alpha-beta) / (alpha + beta) + np.sqrt(
                2 * np.log(self.trials) / self.N[state])

        return max(beyes, key=beyes.get)

    def weighted_score(self, state: OthelloState):
        curPlayer = 0
        oppPlayer = 0
